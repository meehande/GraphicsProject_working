...
public:
	...
	void perspective(float fov, float aspect, float n, float f) {
		const float t = tan(radians(fov*0.5f)) * n;
		const float b = -t;
	...
	void transform(Vector3 &v) const {
		float x = v.x * m11 + v.y * m12 + v.z * m13 + m14;
		float y = v.x * m21 + v.y * m22 + v.z * m23 + m24;
		float z = v.x * m31 + v.y * m32 + v.z * m33 + m34;
		v.x = x;
		v.y = y;
		v.z = z;
	}
	...
	// the convention used is post-multiplication by a column vector: x=Ab.
	void transform(const Vector3 &v, Vector3 &result) const {
	...
	friend inline Vector3 operator*(const Vector3 &matrix, const Vector3 &v);
	friend inline Vector3 operator*(const Vector3 &v, const Matrix &matrix);
	friend inline Vector3& operator*=(Vector3 &v, const Matrix &matrix);
	...
	inline Vector3 operator*(const Matrix &matrix, const Vector3 &v) {
		return Vector3(v.x * matrix.m11 + v.y * matrix.m12 + v.z * matrix.m13 + matrix.m14,
					   v.x * matrix.m21 + v.y * matrix.m22 + v.z * matrix.m23 + matrix.m24,
					   v.x * matrix.m31 + v.y * matrix.m32 + v.z * matrix.m33 + matrix.m34);
	}

	inline Vector3 operator*(const Vector3 &v, const Matrix &matrix)
	{
		// when we premultiply x*A we assume the vector is a row vector
		return Vector3(v.x * matrix.m11 + v.y * matrix.m21 + v.z * matrix.m31 + matrix.m41,
	...
	inline Vector3& operator*=(Vector3 &v, const Matrix &matrix)
	{
		...
		const float rx = v.x * matrix.m11 + v.y * matrix.m12 + v.z * matrix.m13 + matrix.m14;
		const float ry = v.x * matrix.m21 + v.y * matrix.m22 + v.z * matrix.m23 + matrix.m24;
		const float rz = v.x * matrix.m31 + v.y * matrix.m32 + v.z * matrix.m33 + matrix.m34;
		v.x = rx;
		v.y = ry;
		v.z = rz;
		return v;
	...
	//Used for OpenGL, return pointer to address of first element of Matrix
	inline float* value_ptr(Matrix &mat) {
		return &(mat.m11);
	}
}